...
const API_RESIDENTES = 'http://34.125.178.201/app/api/residente/index.php?action=';
...
function isLogged(id,alias,foto,modo,ip) {
    ...
    var api_nav;
    if (id > 0) {
        api_nav = `http://34.125.178.201/app/api/residente/index.php?id=${id}&action=validateSession`;
    } else {
        api_nav = `http://34.125.178.201/app/api/residente/index.php?action=validateSession`;
    }
                        ...
                        <div class="vertical-nav colorCitiger" id="sidebar">
                            ...
                            <div id="tarjeta">
                                <div id="tarjetaPerfil" class="p-3">
                                    <div class="row">
                                        <div class="col-3">
                                            <img src="http://34.125.178.201/resources/img/dashboard_img/residentes_fotos/${foto}"
...
function logOut2() {
    swal({
        ...
        // Se verifica si fue cliqueado el botón Sí para hacer la petición de cerrar sesión, de lo contrario se muestra un mensaje.
        if (value) {
            fetch(`http://34.125.178.201/app/api/residente/index.php?action=logOut&id=${params.get('id')}`, {
                method: 'get'
            }).then(function (request) {
                // Se verifica si la petición es correcta, de lo contrario se muestra un mensaje indicando el problema.
                if (request.ok) {
